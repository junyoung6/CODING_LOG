...
function findRoute() {
    ...
    geocode(startAddress, (startLat, startLon) => {
        ...
        geocode(endAddress, (endLat, endLon) => {
            ...
            // 자동차 경로 계산 및 표시
            calculateRoute(startLatLng, endLatLng, 'driving', 'blue');
            // 도보 경로 계산 및 표시
            calculateRoute(startLatLng, endLatLng, 'foot', 'green');
        });
    });
}

// 출발지와 목적지 좌표를 기반으로 최적 경로를 계산하는 함수 (OSRM API 사용)
function calculateRoute(startLatLng, endLatLng, mode, color) {
...
map.on('dragend', function(e) {
    if (e.target === startMarker || e.target === endMarker) {
        calculateRoute(startMarker.getLatLng(), endMarker.getLatLng(), 'driving', 'blue');
        calculateRoute(startMarker.getLatLng(), endMarker.getLatLng(), 'foot', 'green');
    }
});
